...
Dernière mise à jour: <2016-02-26 ven. 09:31>
...
La valeur par défaut pour la licence est la chaîne vide (pas de droits
d'auteur explicite).

Une valeur standard est "© Jardin de botanique de Fontainebleau"
indiquant à qui appartiennent les droits d'auteur.  En général, ce
champ est identique pour tous les items importés d'une même base.

La valeur par défaut pour le champ Creative Commons est {-1, -1, -1} -
c'est-à-dire "N/A", pour dire que seul le champ "Droits d'auteur" est
pertinent.

# FIXME: la VALEUR PAR DÉFAUT est cc-by-sa-nc

La recommandation provisoire pour les contenus contribués est la
licence [[http://creativecommons.org/licenses/by-nc-sa/3.0/fr/][CC-by-sa-nc]] soit {1,0,1} pour dire "SA s'applique", "ND ne
s'applique pas" et "NC s'applique".
...
** Les droits des contenus collaboratifs
...
# FIXME: PAS de droits sur les contenus collaboratifs
...
de présenter des éléments ensemble.  Elles ne donnent pas lieu à un
droit d'auteur particulier.

# Les utilisateurs peuvent ajouter des éléments à leurs dossiers en
# fonction des droits de ces éléments.

# Les éléments qu'un utilisateur peut ajouter à un dossier sont :

# - les éléments du domaine public ou
# - les éléments dont il est l'auteur ou
# - les éléments sous licence CC-by, CC-by-sa ou CC-by-sa-nc

# La présence d'une clause ND (non-derivative) dans une licence
# interdirait d'insérer un élément dans un dossier (qui en serait
# "dérivé").  Même chose pour un contenu sous droits d'auteur
# classiques.
...
- AOC : les droits du texte intégral ne sont pas dans les métadonnées

- Si on ne moissonne que les métadonnées, cela simplifie la question
  des droits.

- Case à cocher par l'utilisation pour dire qu'il sait ce qu'il fait
  quand il publie un dossier avec plusieurs contenus.

- Dossier avec droits hétéroclites pour les éléments, dans ce cas ce
  dossier n'a pas de licence en tant que telle.

- Est-ce qu'un utilisateur peut être *auteur* d'un dossier ?  NON.
