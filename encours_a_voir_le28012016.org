...
- Profil utilisateur : Possible d'avoir les notifications pour les
  encarts à gauche pour les contenus signalés, les contenus suivis
  (s'ils sont mis à jour), les abonnements (s'il y a eu des mises à
  jour).

- Fusionner mes "contenus suivis" et "mes abonnements" dans une même
  rubrique "Veille" qui contiendra trois onglets (suivi, thèmes et
  utilisateurs).  Le but de la fusion étant d'avoir seulement un flux
  de notification pour la veille en général.

- "Mes notifications" doit être le premier onglet dans la page "Mon
  compte" : on gère les notifications qu'on veut recevoir depuis cette
  page.

- Fusion de la page "statistiques" et du "tableau de bord", et ajout
  des éléments propres aux admins/contributeurs si on est connecté en
  tant qu'admin/contributeur.

- Reprendre dans cet ordre :

  - Mon compte (donnant aussi accès d'abord à l'onglet de notifications)
  - Ma veille
  - Contributions
  - Statistiques
...
- Dans les contenus signalés (disponibles pour les contributeurs et
  ...
  problème : droits d'auteur, contenu inapproprié, problème technique,
...
- Éventuellement utiliser des onglets horizontaux pour les différentes
  statistiques, plutôt que de les empiler [à préciser].

- Dans la page "suivi (1/2)", est-ce que c'est possible de représenter
...
- Dans la page "suivi (1/2)", ajouter un bouton pour créer un dossier
  à partir d'une liste de suivi.

- Dans la listing ajouter un bouton "ajouter au dossier" qui apparaît
  à côté des entrées listées.
